...
# ---------------------------------------------------------
def clean_env(val):
    return unicodedata.normalize("NFKD", val).replace("\xa0", "").strip() if val else ""
...
# High-liquidity universe spanning tech, semiconductors, financial, retail, energy, and indices
UNIVERSE = [
...
def get_alpaca_bars(symbol, timeframe="1Hour", limit=120):
    """Fetches real-time candles directly from Alpaca Market Data API."""
...
def send_digest_email(top_setups):
    ...
    subject = f"🚀 Live Market Digest: Top {len(top_setups)} Price Action & Volatility Setups"
    ...
    <html>
      <body style="font-family: Arial, sans-serif; line-height: 1.5; color: #222; max-width: 950px; margin: auto;">
        <h2 style="color: #1a73e8; border-bottom: 2px solid #1a73e8; padding-bottom: 8px;">
          Live Real-Time Market Scan (Top {len(top_setups)})
        </h2>
    ...
    try:
        ...
        print(f"Top {len(top_setups)} digest successfully sent.")
